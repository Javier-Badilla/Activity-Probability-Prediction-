...
DESCRIPTION:
    Analyzes HEX docking output PDB files (one file per orientation) for
    multiple peptide candidates against a TIP4P ice slab. For each peptide it:

      1. Auto-detects ice residue names in the PDB (including "ICE")
      2. Ranks all orientations by IBS contact quality
      3. Copies the top N orientation PDB files to a top_models folder
      4. Generates per-peptide ranking CSVs and summary plots
      5. Produces a cross-peptide comparison report
